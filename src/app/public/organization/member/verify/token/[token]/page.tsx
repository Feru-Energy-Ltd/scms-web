...
import { verifyOrganizationMemberToken } from "@/lib/api/publicOrg";
import { showApiErrorToast } from "@/lib/toast/showApiErrorToast";
import styles from "@/components/account/ResourceList.module.css";

export default function VerifyOrgMemberTokenPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await verifyOrganizationMemberToken(token);
  ...
  return (
    <main style={{ padding: 24, maxWidth: 820, margin: "0 auto" }}>
      <h1 className={styles.h1}>Organization member verification</h1>
      {loading ? (
